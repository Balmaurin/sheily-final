    ...
    try:
        with open(jsonl_files[0], "r", encoding="utf-8") as f:
            ...
            if data.get("branch") == branch_name:
                ...
                return {
                    "status": "❓",
                    "reason": f"Branch mismatch: {data.get('branch')}",
                    "files": len(jsonl_files),
                    "branch": data.get("branch", "unknown"),
                }
...
def main():
    ...
    for branch_dir in sorted(BASE_PATH.iterdir()):
        ...
        print(
            f"{result['status']} {branch_name:30} | {result['files']:2} files | {result['reason']:30} | branch={result['branch']}"
        )
