...
class TestSafeSubprocessRun:
    ...
    def test_shell_true_is_forced_false(self):
        ...
        result = safe_subprocess_run(["echo", "test"], shell=True, capture_output=True)  # Será forzado a False
        assert result.returncode == 0
    ...
    def test_environment_variables(self):
        ...
        result = safe_subprocess_run(["printenv", "TEST_VAR"], env=env, capture_output=True, text=True)
...
class TestSafeSubprocessPopen:
    ...
    def test_popen_creation(self):
        ...
        proc = safe_subprocess_popen(["echo", "test"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def test_popen_shell_forced_false(self):
        ...
        proc = safe_subprocess_popen(["echo", "test"], shell=True, stdout=subprocess.PIPE)  # Será ignorado
        assert proc is not None
        proc.communicate()
        proc.wait()
...
class TestSubprocessUtilsEdgeCases:
    ...
    def test_special_characters_in_args(self):
        ...
        result = safe_subprocess_run(["echo", "hello-world_test.txt"], capture_output=True, text=True)
        assert result.returncode == 0
    ...
    def test_unicode_in_command(self):
        ...
        result = safe_subprocess_run(["echo", "español_中文"], capture_output=True, text=True)
        assert result.returncode == 0
