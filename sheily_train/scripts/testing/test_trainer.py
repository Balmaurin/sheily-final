...
class TestTrainingSystemReal(unittest.TestCase):
    ...
    def test_mock_components_functionality_real(self):
        ...
        # Test forward pass
        batch = {"input_ids": [[1, 2, 3, 4], [5, 6, 7, 8]], "attention_mask": [[1, 1, 1, 1], [1, 1, 1, 1]]}
    ...
    def test_training_metrics_tracking_real(self):
        ...
        metrics = TrainingMetrics(step=1, epoch=0.1, loss=2.5, learning_rate=1e-4, grad_norm=1.2, perplexity=12.18)
    ...
    def test_metrics_export_and_persistence_real(self):
        ...
        for i in range(5):
            metrics = TrainingMetrics(step=i + 1, epoch=0.1 * (i + 1), loss=3.0 - (i * 0.1), learning_rate=1e-4)
            self.trainer.training_state.metrics_history.append(metrics)
    ...
    def test_integration_with_real_scripts_real(self):
        ...
        if scripts_dir.exists():
            ...
            expected_scripts = ["train_lora.py", "merge_lora.py", "generate_dataset_local.py"]
