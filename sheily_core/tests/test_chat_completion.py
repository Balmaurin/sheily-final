...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "length",
            True,
            "This is not a chat template, it is",
        ),
        (
            "codellama70b",
            "You are a coding assistant.",
            "Write the fibonacci function in c++.",
            128,
            "(Aside|she|felter|alonger)+",
            104,
            128,
            "length",
            False,
        ...
        (
            None,
            "Book",
            [{"type": "text", "text": "What is"}, {"type": "text", "text": "the best book"}],
            8,
            "Whillicter",
            79,
            8,
...
def test_chat_completion(
    ...
    assert match_regex(
        re_content, choice["message"]["content"]
    ), f'Expected {re_content}, got {choice["message"]["content"]}'
...
def test_chat_completion_stream(
    ...
    for i, data in enumerate(res):
        if data["choices"]:
            _validate_stream_choice(data, i, content, re_content, finish_reason, last_cmpl_id)
...
def _validate_stream_choice(data, index: int, content: str, re_content: str, finish_reason: str, last_cmpl_id):
    ...
    if last_cmpl_id is not None:
        assert last_cmpl_id == data["id"]
    if choice["finish_reason"] in ["stop", "length"]:
        assert "content" not in choice["delta"]
        assert utils.match_regex(re_content, content)
        assert choice["finish_reason"] == finish_reason
    else:
...
def test_chat_template_assistant_prefill(prefill, re_prefill):
    ...
    assert (
        res.body["__verbose"]["prompt"]
        == f"<s> <|start_header_id|>system<|end_header_id|>\n\nBook<|eot_id|><|start_header_id|>user<|end_header_id|>\n\nWhat is the best book<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n{re_prefill}"
    )
...
def test_apply_chat_template():
    ...
    assert (
        res.body["prompt"]
        == "<|START_OF_TURN_TOKEN|><|SYSTEM_TOKEN|>You are a test.<|END_OF_TURN_TOKEN|><|START_OF_TURN_TOKEN|><|USER_TOKEN|>Hi there<|END_OF_TURN_TOKEN|><|START_OF_TURN_TOKEN|><|CHATBOT_TOKEN|>"
    )
...
@pytest.mark.parametrize(
    "jinja,json_schema,n_predicted,re_content",
    [
        (False, {"const": "42"}, 6, '"42"'),
        (True, {"const": "42"}, 6, '"42"'),
    ],
...
@pytest.mark.parametrize(
    "jinja,grammar,n_predicted,re_content",
    [
...
def _validate_logprobs_token(token):
    """Validar token con logprobs"""
    assert token.logprob <= 0.0
    assert token.bytes is not None
...
def _process_stream_choice(choice, index: int, output_text: str, aggregated_text: str):
    """Procesar choice en stream de logprobs - retorna (nuevo_output, nuevo_aggregated)"""
    if index == 0:
        assert choice.delta.content is None
        assert choice.delta.role == "assistant"
    ...
    for token in choice.logprobs.content:
        aggregated_text += token.token
        _validate_logprobs_token(token)
    return output_text, aggregated_text
...
def test_context_size_exceeded_stream():
    ...
    server.start()
    try:
        for _ in server.make_stream_request(
            "POST",
            "/chat/completions",
            data={
                "messages": [
...
def test_return_progresssss(n_batch, batch_count, reuse_cache):
    ...
    def make_cmpl_request():
        return server.make_stream_request(
            "POST",
            "/chat/completions",
            data={
                "max_tokens": 10,
