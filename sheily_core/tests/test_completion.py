...
server = utils.ServerPreset.tinyllama2()
...
def test_completion(
    ...
    assert isinstance(res.body["has_new_line"], bool)
    assert utils.match_regex(re_content, res.body["content"])
    if return_tokens:
        assert len(res.body["tokens"]) > 0
        assert all(isinstance(tok, int) for tok in res.body["tokens"])
    else:
...
def test_completion_stream(
    ...
    content = ""
    for data in res:
        assert "stop" in data and isinstance(data["stop"], bool)
        if data["stop"]:
            assert data["timings"]["prompt_n"] == n_prompt
            assert data["timings"]["predicted_n"] == n_predicted
            assert data["truncated"] == truncated
            assert data["stop_type"] == "limit"
            assert isinstance(data["has_new_line"], bool)
    ...
    for _ in range(4):
        res = server.make_request(
            ...
            data={
                ...
                "seed": 42,
                "temperature": 0.0,
                "cache_prompt": False,  # NOTA: cache_prompt deshabilitado temporalmente
            },
        )
...
# NOTA: temperature=1.0 produce resultados variables - usar solo 0.0
...
def test_json_prompt_mtm_error_when_not_supported():
    ...
    res = server.make_request(
        ...
        data={
            "prompt": {
                JSON_PROMPT_STRING_KEY: "I believe the meaning of life is <__media__>",
                JSON_MULTIMODAL_KEY: "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=",
            },
            "seed": 42,
            "temperature": 1.0,
            "cache_prompt": False,
...
def test_completion_parallel_slots(n_slots: int, n_requests: int):
    ...
    for i in range(n_requests):
        prompt, _ = PROMPTS[i % len(PROMPTS)]
        tasks.append(
            (
                server.make_request,
                (
    ...
    # check results
    for i in range(n_requests):
        _, _ = PROMPTS[i % len(PROMPTS)]
        res = results[i]
        assert res.status_code == 200
        assert isinstance(res.body["content"], str)
        assert len(res.body["content"]) > 10
        # NOTA: resultado no determinista cuando se usa slot != 0
        # assert utils.match_regex(re_content, res.body["content"])
...
def test_n_probs():
    ...
    for tok in res.body["completion_probabilities"]:
        ...
        assert "token" in tok and isinstance(tok["token"], str)
        assert "logprob" in tok and tok["logprob"] <= 0.0
        assert "bytes" in tok and isinstance(tok["bytes"], list)
        assert len(tok["top_logprobs"]) == 10
        for prob in tok["top_logprobs"]:
            assert "id" in prob and prob["id"] > 0
            assert "token" in prob and isinstance(prob["token"], str)
            assert "logprob" in prob and prob["logprob"] <= 0.0
            assert "bytes" in prob and isinstance(prob["bytes"], list)


def _validate_token_prob(tok, n_probs: int):
    """Validar token con probabilidades"""
    assert "id" in tok and tok["id"] > 0
    assert "token" in tok and isinstance(tok["token"], str)
    assert "logprob" in tok and tok["logprob"] <= 0.0
    assert "bytes" in tok and isinstance(tok["bytes"], list)
    assert len(tok["top_logprobs"]) == n_probs
    for prob in tok["top_logprobs"]:
        assert "id" in prob and prob["id"] > 0
        assert "token" in prob and isinstance(prob["token"], str)
        assert "logprob" in prob and prob["logprob"] <= 0.0
...
def test_n_probs_post_sampling():
    ...
    for tok in res.body["completion_probabilities"]:
        ...
        for prob in tok["top_probs"]:
            ...
            assert "bytes" in prob and isinstance(prob["bytes"], list)
        # porque el modelo de test suele dar probabilidades de 100% o 0%, verificamos todos los top_probs
