...
class UniversalManager:
    ...
    def _initialize_model(self) -> Dict[str, Any]:
        ...
        # Cargar tokenizer
        self.tokenizer = AutoTokenizer.from_pretrained(model_config["base_model"], trust_remote_code=True)
        ...
        if current_adapter.exists() and (current_adapter / "adapter_config.json").exists():
            ...
            base_model = AutoModelForCausalLM.from_pretrained(
                model_config["base_model"], trust_remote_code=True, torch_dtype=torch.float32
            )
            self.model = PeftModel.from_pretrained(base_model, str(current_adapter), is_trainable=True)
    ...
    def _process_incoming_data(self) -> Dict[str, Any]:
        ...
        if not incoming_files:
            return {"status": "no_incoming_data", "files_processed": 0}
        ...
        return {"status": "processed", "files_processed": len(processed), "files": processed}
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status["corpus"] = {"files": len(corpus_files), "total_documents": total_docs}
        ...
        if self.model is not None:
            ...
            status["model"] = {"loaded": True, "trainable_params": trainable, "total_params": total}
        else:
    ...
    def build_rag_index(self) -> Dict[str, Any]:
        ...
        st_model = SentenceTransformer(
            self.config["rag"]["embedding_model"], device="cpu"  # Usar CPU para no interferir con entrenamiento
        )
        st_embeddings = st_model.encode(texts, show_progress_bar=True, batch_size=32)
    ...
    def search_rag(self, query: str, top_k: int = 5, method: str = "hybrid") -> List[Dict[str, Any]]:
            ...
            if tfidf_path.exists():
                ...
                for idx in top_indices:
                    if similarities[idx] > 0:
                        results.append(
                            {"document": documents[idx], "score": float(similarities[idx]), "method": "tfidf"}
                        )
        ...
        if method in ["semantic", "hybrid"]:
            ...
            if st_embeddings_path.exists():
                ...
                st_model = SentenceTransformer(self.config["rag"]["embedding_model"], device="cpu")
                query_embedding = st_model.encode([query])[0]

                # Calcular similitud
                from sklearn.metrics.pairwise import cosine_similarity

                similarities = cosine_similarity(query_embedding.reshape(1, -1), st_embeddings)[0]
                ...
                for idx in top_indices:
                    if similarities[idx] > self.config["rag"]["min_score"]:
                        results.append(
                            {"document": documents[idx], "score": float(similarities[idx]), "method": "semantic"}
                        )
        ...
        if method == "hybrid" and len(results) > 0:
            ...
            results = [
                {"document": v["document"], "score": v["score"], "methods": v["methods"]} for v in combined.values()
            ]
    ...
    def add_knowledge(self, data_source: Path, auto_train: bool = False) -> Dict[str, Any]:
        ...
        result = {"status": "added", "source": str(data_source), "examples": examples, "target": str(target)}
