...
def main():
    parser = argparse.ArgumentParser(
        description="Añade conocimiento al Sistema Universal Sheily"
    )
    parser.add_argument(
        "dataset",
        type=str,
        help="Ruta al archivo JSONL con los datos"
    )
    parser.add_argument(
        "--auto-train",
        action="store_true",
        help="Entrenar automáticamente después de añadir"
    )
    parser.add_argument(
        "--domain",
        type=str,
        default="general",
        help="Dominio de origen (solo informativo, opcional)"
    )
