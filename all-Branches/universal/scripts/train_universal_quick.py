...
def prepare_small_dataset(corpus_path: Path, tokenizer, max_examples: int = 100):
    ...
    def tokenize_function(examples):
        ...
        return tokenizer(texts, truncation=True, max_length=512, padding=False)  # Más corto para ser rápido

    logger.info("🔧 Tokenizando...")
    tokenized = dataset.map(tokenize_function, batched=True, remove_columns=dataset.column_names, desc="Tokenizando")
...
def train_quick(manager: UniversalManager, max_examples: int = 100, epochs: int = 1):
    ...
    dataset = prepare_small_dataset(manager.corpus_path, manager.tokenizer, max_examples=max_examples)
    ...
    optimizer = AdamW(manager.model.parameters(), lr=2e-4, foreach=False)  # Crítico para DirectML
...
def main():
    parser = argparse.ArgumentParser(description="Entrenamiento rápido de prueba del Sistema Universal")
    parser.add_argument("--max-examples", type=int, default=100, help="Número máximo de ejemplos (default: 100)")
    parser.add_argument("--epochs", type=int, default=1, help="Número de épocas (default: 1)")
    ...
    # Entrenar
    result = train_quick(manager, max_examples=args.max_examples, epochs=args.epochs)
