...
def prepare_dataset(corpus_path: Path, tokenizer, max_length: int = 512):
    ...
    tokenized = unified_dataset.map(
        tokenize_function,
        batched=True,
        remove_columns=unified_dataset.column_names,
        desc="Tokenizando"
    )
...
def train_universal_adapter(
    ...
    dataset = prepare_dataset(
        manager.corpus_path,
        manager.tokenizer,
        max_length=manager.config["model"]["max_length"]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Entrena el adaptador universal de Sheily"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=2,
        help="Tamaño del batch (default: 2)"
    )
    parser.add_argument(
        "--epochs",
        type=int,
