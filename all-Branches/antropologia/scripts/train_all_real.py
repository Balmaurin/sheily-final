...
class RealIncrementalTrainingPipeline:
    ...
    def train_with_dataset_real(
        ...
        def tokenize_no_padding(batch):
            ...
            output = tokenizer(batch["text"], truncation=False, add_special_tokens=False)
        ...
        def group_texts(examples):
            ...
            return {"input_ids": chunks, "labels": chunks}  # Para causal LM

        # Pre-tokenizar y empaquetar (1 sola vez)
        logger.info("Pre-tokenizando con packing (sin padding)...")
        tokenized_dataset = dataset.map(
            tokenize_no_padding, batched=True, remove_columns=dataset.column_names, desc="Tokenizando"
        )

        tokenized_dataset = tokenized_dataset.map(group_texts, batched=True, desc="Empaquetando")
        ...
        data_collator = DataCollatorForLanguageModeling(
            tokenizer=tokenizer, mlm=False, pad_to_multiple_of=None  # No padding
        )
    ...
    def train_all_real(
        ...
        try:
            model, tokenizer = self.load_or_create_model_and_tokenizer(
                base_model, adapter_path if (adapter_path / "adapter_config.json").exists() else None
            )
        except Exception as e:
        ...
        for idx, dataset in enumerate(prioritized, 1):
            ...
            try:
                metadata = self.train_with_dataset_real(
                    dataset["path"], model, tokenizer, adapter_path, batch_size=batch_size, epochs=epochs_per_dataset
                )

                training_history.append(metadata)
                results["datasets_trained"].append(
                    {"dataset": dataset["name"], "status": "success", "metadata": metadata}
                )
