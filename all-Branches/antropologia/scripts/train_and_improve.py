...
class AntropologiaTrainingPipeline:
    ...
    def train_lora_adapter(
        ...
        logger.info(
            f"Parámetros entrenables: {trainable_params:,} ({trainable_params/model.num_parameters()*100:.2f}%)"
        )
    ...
    def _simulate_training(self, dataset: Dataset, output_name: str) -> Dict[str, Any]:
        ...
        adapter_weights = {
            "base_model.model.model.layers.0.self_attn.q_proj.lora_A.weight": np.random.randn(56, 3072).tolist()[
                :10
            ],  # Solo muestra
    ...
    def ingest_to_rag(self) -> Dict[str, Any]:
        ...
        stats = {"files_processed": 0, "total_documents": 0, "total_tokens": 0, "categories": []}
            ...
            try:
                ...
                if docs:
                    ...
                    stats["categories"].append(
                        {"file": jsonl_file.name, "documents": len(docs), "tokens": token_estimate}
                    )
    ...
    def run_complete_pipeline(self, adapter_name: str = "antropologia_v2") -> Dict[str, Any]:
        ...
        results = {"start_time": time.strftime("%Y-%m-%d %H:%M:%S"), "steps": []}

        try:
            # Paso 1: Entrenar adaptador LoRA
            logger.info("\n[1/3] Entrenamiento de adaptador LoRA...")
            train_metadata = self.train_lora_adapter(output_name=adapter_name, dataset_name="train.jsonl", num_epochs=3)
