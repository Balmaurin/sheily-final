...
def load_model_with_adapter(base_model: str = "TinyLlama/TinyLlama-1.1B-Chat-v1.0", adapter_path: str = None):
...
def generate_answer(model, tokenizer, device, question: str, max_length: int = 300) -> str:
...
def main():
    ...
    # Cargar modelo
    model, tokenizer, device = load_model_with_adapter(adapter_path=str(adapter_path))
    ...
    for i, test in enumerate(EVALUATION_QUESTIONS, 1):
        ...
        results.append(
            {"level": test["level"], "question": test["question"], "answer": answer, "evaluation": evaluation}
        )
