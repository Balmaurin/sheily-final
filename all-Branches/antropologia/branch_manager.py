...
class AntropologiaManager:
    ...
    def _validate_structure(self) -> None:
        ...
        """
        essential_dirs = [self.corpus_path, self.adapters_path, self.training_path]
    ...
    def load_adapters(self, adapter_type: str = "current") -> Dict[str, Any]:
        ...
        adapter_info = {"type": adapter_type, "path": str(adapter_dir), "loaded": False}
        ...
        if metadata_path.exists():
            try:
                ...
                logger.info(
                    f"Metadata del adaptador cargada: quality_score={adapter_info['metadata'].get('quality_score')}"
                )
            except Exception as e:
                logger.error(f"Error cargando metadata del adaptador: {e}")
    ...
    def initialize_rag(self) -> Dict[str, Any]:
        ...
        rag_info = {"initialized": False, "indices": {}}
    ...
    def load_memory(self) -> Dict[str, Any]:
        ...
        memory_info = {"loaded": False, "integrators": []}
        ...
        if memory_integrator_path.exists():
            memory_info["integrators"].append(
                {"name": "memory_integrator", "path": str(memory_integrator_path), "available": True}
            )
        ...
        if lora_rag_integrator_path.exists():
            memory_info["integrators"].append(
                {"name": "lora_rag_integrator", "path": str(lora_rag_integrator_path), "available": True}
            )
    ...
    def get_specialized_model(self) -> Dict[str, Any]:
        ...
        if not self.model_config_path.exists():
            ...
            return {"available": False, "message": "Configuración del modelo no encontrada"}
    ...
    def train_branch_specific(
        ...
        training_result = {"status": "prepared", "data_path": data_path, "domain": "antropologia"}
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self._corpus_data:
            total_docs = sum(
                len(docs)
                for corpus in self._corpus_data.values()
                for key, docs in corpus.items()
                if key != "_metadata" and isinstance(docs, list)
            )
            status["corpus_stats"] = {"types_loaded": len(self._corpus_data), "total_documents": total_docs}

        # Estadísticas de adaptadores
        if self._adapters:
            status["adapter_stats"] = {"types_loaded": len(self._adapters), "adapters": list(self._adapters.keys())}
    ...
    def initialize_all(self) -> Dict[str, Any]:
        ...
        results = {"branch": self.branch_name, "success": True, "components": {}}

        # Cargar corpus
        try:
            corpus_data = self.load_corpus("spanish")
            results["components"]["corpus"] = {"status": "success", "categories": len(corpus_data)}
        except Exception as e:
        ...
        try:
            ...
            results["components"]["adapter"] = {"status": "success", "loaded": adapter_info.get("loaded", False)}
        except Exception as e:
        ...
        try:
            ...
            results["components"]["memory"] = {"status": "success", "loaded": memory_info.get("loaded", False)}
        except Exception as e:
        ...
        try:
            ...
            results["components"]["model"] = {"status": "success", "available": model_config.get("available", False)}
        except Exception as e:
