...
class AntropologiaLoRARAGIntegrator:
    ...
    def get_domain_specific_adapters(self) -> Dict[str, Any]:
        ...
        adapters = {"domain": self.domain, "available": {}, "recommended": None}

        # Tipos de adaptadores a buscar
        adapter_types = ["current", "previous", "functional", "optimized", "complete_optimized", "retraining"]
    ...
    def get_domain_rag_config(self) -> Dict[str, Any]:
        ...
        if corpus_training.exists():
            rag_config["corpus"]["training"] = {"path": str(corpus_training), "available": True}
        ...
        for index_name, index_path in indices.items():
            if index_path.exists():
                rag_config["indices"][index_name] = {"path": str(index_path), "available": True}
        ...
        rag_config["configuration"]["embedding_model"] = (
            rag_config.get("configuration", {})
            .get("corpus_metadata", {})
            .get("embedding_model", "sentence-transformers/all-mpnet-base-v2")
        )
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        status = {"domain": self.domain, "branch_path": str(self.branch_path), "components": {}}
        ...
        try:
            ...
            status["components"]["adapters"] = {"status": "error", "error": str(e)}
        ...
        status["integration_ready"] = (
            status["components"]["adapters"].get("status") == "available"
            and status["components"]["rag"].get("status") == "available"
        )
    ...
    def load_recommended_adapter(self) -> Optional[Dict[str, Any]]:
        ...
        return {"name": recommended, "info": adapter_info, "domain": self.domain}
